...
public class ScopedContainerTest
{
	...
	public void CreateScopeWithResolveOpenGeneric()
	{
		...
		Assert.That(singleton, Is.Not.SameAs(singleton2));
		Assert.That(singleton3, Is.SameAs(singleton4));
	...
	public void ResolveCollectionFromParent()
	{
		...
		var childContainer = parentContainer.CreateScope(childBuilder => { childBuilder.Register<I1CollectionService>(Lifetime.Scoped); });
	...
	public void ResolveCollection_ElementRepeatedly()
	{
		...
		var childContainer = parentContainer.CreateScope(childBuilder => { childBuilder.Register<HasInstanceId>(Lifetime.Singleton); });
	...
	public void Inject()
	{
		var builder = new ContainerBuilder();
		var container = builder.Build();
		var childContainer = container.CreateScope(childBuilder => { childBuilder.Register<I2, NoDependencyServiceA>(Lifetime.Singleton); });
